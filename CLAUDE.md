...
2. **查询场景**
   - 示例："查找所有API接口"、"项目结构是什么？"
   - 处理：使用搜索工具查询并返回结果

3. **分析场景**
   - 示例："分析这个bug的原因"、"评估性能瓶颈"
   - 处理：分析并给出建议，不直接修改代码
...
**场景1：复杂任务**
```
用户：在 mall-portal 和 beilv-agent 中添加积分扣减功能

主代理：
我需要为两个项目添加积分扣减功能。让我先分析任务复杂度...

[使用 EnterPlanMode]

任务分析：
- 类型：新功能开发
- 影响范围：2个项目，涉及数据库、API、前端
- 复杂度：复杂

请选择执行方式：
1. 完整工作流（推荐）- 包含分析、计划、测试、审计
2. 快速执行 - 直接实现代码

用户：选择完整工作流

主代理：好的，我将启动完整的多代理工作流...
[开始阶段0：创建会话]
```

**场景2：简单任务**
```
用户：修改配置文件中的端口号从8080改为3000
