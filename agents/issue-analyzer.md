...
**识别标志**：
- 文件中包含代码块（如 \`\`\`python、\`\`\`javascript 等）
- 包含具体的文件路径和行号（如"app/xxx/yyy.py 第 123 行"）
- 包含详细的实施步骤或方法定义
...
### 步骤2.8：计划审查（仅在模式 B 时执行）

**⚠️ 此步骤仅在检测到用户提供详细技术方案时执行**

如果用户提供的需求文件包含详细的技术实施方案（如代码示例、具体步骤、文件路径和行号），需要进行计划审查。

#### 2.8.1 识别是否需要审查

检查用户需求中是否包含：
- 代码块（\`\`\`python、\`\`\`javascript、\`\`\`java 等）
- 具体的文件路径和行号（如"app/xxx/yyy.py:123"）
- 详细的方法签名和参数说明
- 数据结构设计（如 Redis 命令、数据库表结构）
- 具体的实施步骤编号
...
- 数据结构选择是否恰当？
- 是否有明显的性能瓶颈？
- 是否存在并发安全问题？（如 Redis Pipeline 是否需要事务）
- API 调用是否正确？参数是否完整？

**2. 架构一致性审查**
- 是否符合项目现有架构？（基于 project.info）
- 文件路径和模块划分是否合理？
- 是否遵循项目编码规范？
- 命名风格是否一致？
...
在分析报告中增加"计划审查"章节（见步骤7的报告模板）。

**关键原则**：
- 建设性：不仅指出问题，更要提供解决方案
- 具体性：明确指出问题位置（行号、文件路径）
- 优先级：区分必须修复和建议改进
...
**⚠️ 本章节仅在检测到用户提供详细技术方案时输出**

### 审查模式
{模式 A：简单需求分析 | 模式 B：详细计划审查}

### 审查摘要
- {✅ 技术方案整体可行 | ⚠️ 发现需要注意的问题}
- 发现问题数：{N} 个（高：{X}，中：{Y}，低：{Z}）
- 优化建议数：{M} 个

### 发现的问题

#### 问题 1：{问题标题}（严重性：高/中/低）
**位置**：原计划第 {X}-{Y} 行 / 文件 `{file_path}:行号`
**问题描述**：{详细说明技术问题}
**影响**：{可能的负面影响}
**建议修复**：{具体的修复方案}
